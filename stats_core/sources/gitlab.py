...
class GitLabSource(BaseSource):
    ...
    def _request(self, path: str, params: dict | None = None) -> Any:
        ...
        logger.debug(
            "GitLab API request: %s | headers: %s | params: %s",
            url,
        ...
        try:
            ...
            logger.debug("GitLab API response: %s %s", resp.status_code, resp.reason)
            ...
            if hasattr(exc, "response") and exc.response is not None:
                try:
                    ...
                    pass
            logger.error(
                "GitLab API error for %s | params: %s | error: %s | response: %s",
                url,
                params or {},
                exc,
                response_text,
