...
CONFIG_POINT_LOCAL = "gitlab"
CONFIG_POINT_GLOBAL = "global"
CONFIG_FILE = "config.ini"
CONFIG_BASE_URL = "gitlab-url"
CONFIG_TOKEN = "token"
...
def main():
    ...
    tool_description = 'Track commit by using Gitlab API'
    parser = argparse.ArgumentParser(description=tool_description, formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('-t', '--token', dest='token', help='token')
    parser.add_argument('-l', '--url-link', dest='base_url', help='the gitlab url')
    parser.add_argument('-f', '--file', dest='member_list_file', help='Group member list file')
    parser.add_argument('-b', '--branch', dest='branch', help='Branch to get report for all projects. Can be comma separated list like "master,dev,main"')
    parser.add_argument('-p', '--project', dest='project', help='Specify project for report path with namespace. Possible to specify several project separated by comma')
    parser.add_argument('-d', '--date', dest='date_since', required=True, help='query date string since, eg. 2019-06-25')
    parser.add_argument('-u', '--until', dest='date_until', required=False, help='query date string until, eg. 2019-12-25')
    ...
    branch_list = options.branch.split(",")
    ...
    for user in member_list:
        ...
        for project_id in ids:
            project_info = get_project_info(base_url, s, project_id)
            print('(' + str(idx) + '/' + str(project_num) + ')Analyzing project: ' + project_info['path_with_namespace'] + ' For Username: ' + author)
            for branch in branch_list:
                commits = get_all_commits(base_url, s, project_id, branch, since, until, author)
                for c in commits:
                    detail = get_commit_detail(base_url, s, project_id, c['id'])
                    stats = detail['stats']
                    name = detail['author_name'] ### TODO Gitlab returns ID insteado of a name
                    title = detail['title']
                    commit_url = detail['web_url']
                    commit_date = detail['committed_date']
                    description = detail['message']
                    additions = stats['additions']
                    deletions = stats['deletions']
                    total = stats['total']
                    comments = get_commit_comments_count(base_url, s, project_id, c['id'], member_list, name)
...
def get_commit_comments_count(base_url, session, project_id, commit_id, member_list, commit_author):
    ...
    for comment in resp.json():
        if commit_author != comment['author']['username']:
            filter_result = list(filter(lambda author: author['Username'] == comment['author']['username'], member_list))
            if filter_result:
                inner += 1
...
def create_csv_file(file_name):
    ...
    with open(file_name, "w", encoding='utf-8-sig', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(["Name", "Project", "Date", "Gitlab id", "branch", "description", "LOC", "Reviewed By", "AR", "change_inner_group_comments_count", "change_outer_group_comments_count"])

def get_ticketno(description):
    ticket_pattern = re.compile('\[(issue-\d{1,5})\]')
