...
def main():
    ...
    for user in member_list:
        ...
        for project_id in ids:
            ...
            print('(' + str(idx) + '/' + str(project_num) + ')Analyzing project: ' + project_info[
                'path_with_namespace'] + ' For Username: ' + author)
...
def create_csv_file(file_name):
    ...
    with open(file_name, "w", encoding='utf-8-sig', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(["Name", "Project", "Date", "Gitlab id", "branch", "description", "LOC", "Reviewed By",
                         "change_inner_group_comments_count", "change_outer_group_comments_count"])


def get_ticketno(description):
    ticket_pattern = re.compile('\[(\d{1,5})\]')
